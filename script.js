...
$(function() {
    ...
    function rates2(e) {
        ...
        var curRates = [];
        /*rate(); if (item["Cur_ID"] == 345) */
        var curIdUzs = 345;
        $.getJSON(uri + 'ExRates/Rates/' + curIdUzs, { 'onDate':new Date($('#iDate').val()).toUTCString(), 'ParamMode': 0 })
        .done(function (data) {
...
function crossCur(curRates, e) {
    ...
    if (e == "U") {
        ...
        document.getElementById('UZS').value = (document.getElementById('USD').value * usrId / eurID * eurId * 10000 / uzsId).toFixed(4);
    ...
    if (e == "R") {
        ...
        document.getElementById('UZS').value = (document.getElementById('RUB').value * rubId / 100 / uzsID * 10000).toFixed(4);
    ...
    if (e == "P") {
        ...
        document.getElementById('UZS').value = (document.getElementById('PLN').value * plnId / 10 / uzsId * 1000).toFixed(4);
    ...
    if (e == "K") {
        ...
        document.getElementById('UZS').value = (document.getElementById('KZT').value * kztId / 1000 / uzsId *10000).toFixed(4);
...
//----Для получения одной валюты по коду. Буду использовать для узбекского сума  //
  ...
  function rate() {
    $.getJSON(uri + 'ExRates/Rates/' + $('#iCur').val(), { 'onDate': parseRuDate($('#iDate').val()).toUTCString(), 'ParamMode': 0 })
  .done(function (data) {
